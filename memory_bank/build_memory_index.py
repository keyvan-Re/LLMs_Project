...
def generate_memory_docs(data,language):
    ...
    for user_name, user_memory in data.items():
        ...
        for date, content in user_memory['history'].items():
            memory_str = f'Conversation on {date}：' if language=='cn' else f'Conversation on {date}：'
            ...
            if 'summary' in user_memory.keys():
                if date in user_memory['summary'].keys():
                    summary = f'The summary of the conversation on {date} is：{user_memory["summary"][date]}' if language=='cn' else f'The summary of the conversation on {date} is: {user_memory["summary"][date]}'
                    memory_str += summary

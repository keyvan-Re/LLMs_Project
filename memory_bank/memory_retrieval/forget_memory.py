        ...
        with open(self.filepath, "r", encoding="utf-8") as f:
            ...
            for user_name, user_memory in memories.items():
                ...
                for date, content in user_memory['history'].items():
                     memory_str = f'rigin {date} type:' if self.language=='cn' else f'Conversation content on {date}:'
                     ...
                     if 'summary' in self.memory_bank[user_name].keys():
                        if date in self.memory_bank[user_name]['summary'].keys():
                            ...
                            summary = f'speed{date}indefinitely：{summary_str}' if self.language=='cn' else f'The summary of the conversation on {date} is: {summary_str}'

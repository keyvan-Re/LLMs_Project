    ...
    def load(self,name):
        ...
        # print(user_memory)
            ...
            for date, content in user_memory['history'].items():
                ...
                memory_str = f'Conversation content on {date}:' if self.language=='cn' else f'Conversation content on {date}:'
                ...
                if 'summary' in user_memory.keys(): 
                    if date in user_memory['summary'].keys():
                        summary = f'The summary of the conversation on {date} is：{user_memory["summary"][date]}' if self.language=='cn' else f'The summary of the conversation on {date} is: {user_memory["summary"][date]}'
                        # memory_str += summary
...
class LocalMemoryRetrieval:
    ...
    def init_memory_vector_store(self,
        ...
        if len(docs) > 0:
            ...
            print("If you want to be a friend of mine, please make sure you have a friendly friend")
    ...
    def search_memory(self,
        ...
        for doc in related_docs:
            doc.page_content = doc.page_content.replace(f' type{doc.metadata["source"]}value type:','').strip()
            if doc.metadata["source"] != pre_date:
                # date_docs.append(f' type field {doc.metadata["source"]} input type:{doc.page_content}')
        ...
        # memory_contents = [f' public key'+doc.metadata['source']+'source_source:'+doc.page_content for doc in related_docs]

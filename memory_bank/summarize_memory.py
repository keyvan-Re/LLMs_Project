...
def summarize_overall_personality(content, language='en'):
    if language == 'cn':
        prompt = ('The following are the user\'s exhibited personality traits and emotions throughout multiple dialogues, '
        ...
        prompt = ("The following are the user's exhibited personality traits and emotions throughout multiple dialogues, "
                  "along with appropriate response strategies for the current situation:")
        summary_suffix = "Please provide a highly concise and general summary of the user's personality and the most appropriate response strategy for the AI lover, summarized as:"

    for date, summary in content:
        prompt += f"At {date}, the analysis shows {summary.strip()}"

    prompt += summary_suffix
    return prompt
